...
fn modify(v: &mut Vec<i32>) {
    v.push(42); // Can mutate but...
                // v is DROPPED here - no ownership taken!
}
fn read(v: &Vec<i32>) {
    println!("Reading vector: {:?}", v);
}
...
fn main() {
    ...
    let mut x = vec![1, 2, 3]; // Ownership here

    // Mutable borrow: NO ownership transfer
    modify(&mut x);
    read(&x);
